// <copyright file="ICosmosDbSqlClient.cs" company="Endjin Limited">
...
namespace Corvus.Extensions.CosmosDb
{
    ...
    /// and reused for all access for this particular client configuration in a particular host instance.
    /// </para>
    /// <para>
    /// Note that this is intrinsically a single tenant. Multi-tenancy support is layered
    /// on top using the related configuration and tenancy libraries.
    /// </para>
    /// <para>
    /// The <see cref="ICosmosDbSqlClient"/> itself offers a very simple set of APIs for <see cref="UpsertAsync{T}(T, RequestOptions)"/>,
    ...
    /// </para>
    /// <para>
    /// To make them easier to work with, we provide a number of extension methods in <see cref="CosmosDbSqlClientExtensions"/> which can convert from a <see cref="Document"/> to a particular concrete type,
    ...
    /// (via the <see cref="CosmosDbSqlClientDefinition"/> and <see cref="ICosmosDbSqlClientConfiguration"/> types).
    /// You can opt in to this by using the <see cref="CosmosDbSqlClientServiceCollectionExtensions.AddCosmosDbSqlClientJsonConverters"/> extension
    /// to configure the custom <see cref="JsonConverter"/>.
    /// </para>
    /// <para>
    /// You can store multiple different document types in the same client - the results are only coerced to a particular dotnet type
    ...
    /// </para>
    /// <para>
    /// When creating the <see cref="ICosmosDbSqlClient"/> you provide an additional parameter to specify that you would like to create the database with DB level throughput.
    /// </para>
    /// <para>
    ...
    /// <example>
    /// <para>
    /// First, construct an instance of the client. In this example, we are configuring 1000RU/s, and a direct connection. We are also using endjin's custom serialization support for content type resolution. If the database and collection do not exist,
    /// then they will be created for you. The Cosmos DB instance itself must already exist.
    /// </para>
    /// <para>
    /// <code>
    ///  var client = new CosmosDbSqlClient("ADD_YOUR_DATABASE_NAME_HERE", "ADD_YOUR_CONTAINER_NAME_HERE", new Uri("ADD_YOUR_COSMOS_DB_INSTANCE_URI_HERE"), "ADD_YOUR_CONNECTION_STRING_HERE", SerializerSettings.CreateSerializationSettings(), connectionPolicy: new ConnectionPolicy { ConnectionMode = ConnectionMode.Direct }, defaultOfferThroughput: 10000);
    /// </code>
    /// </para>
    /// <para>
    /// Then we have a couple of different POCOs we want to store in the client. Note that they are not based on the same type.
    ...
    /// </para>
    /// <para>
    /// Now, if we want to add a couple of instances of that type to the client we simply call <see cref="UpsertAsync{T}(T, RequestOptions)"/>.
    /// </para>
    /// <para>
    /// <code>
    /// await client.UpsertAsync(new FirstClass { Id = "1", SomeValue = "hello" });
    /// await client.UpsertAsync(new FirstClass { Id = "2", SomeValue = "dolly" });
    /// await client.UpsertAsync(new SecondClass { Id = "3", SomeValue = 1 });
    /// await client.UpsertAsync(new SecondClass { Id = "4", SomeValue = 2 });
    /// </code>
    /// </para>
    /// <para>
    ...
    /// <para>
    /// <code>
    /// var firstClass = await client.ReadDocumentAsync&lt;FirstClass&gt;("1");
    /// var secondClass = await client.ReadDocumentAsync&lt;SecondClass&gt;("2");
    /// </code>
    /// </para>
    /// <para>
    ...
    /// var feedResponse = await client.ExecuteQueryAsync&lt;FirstClass&gt;(querySpec);
    /// </code>
    /// </para>
    /// <para>
    /// We can also force an insert rather than an upsert with <see cref="CosmosDbSqlClientExtensions.InsertAsync{T}(ICosmosDbSqlClient, T, RequestOptions)"/>.
    /// </para>
    /// <para>
    /// <code>
    /// await client.InsertAsync(new FirstClass { Id = "5", SomeValue = "cheerio" });
    /// </code>
    /// </para>
    /// <para>
    ...
    /// <para>
    /// <code>
    /// var entityInstance = await client.GetEntityInstanceAsync("4");
    ...
    /// await client.UpdateAsync(entityInstance);
    ...
    /// // This succeeds as the etags match (assuming no-one else has updated it behind our backs)
    /// await client.UpdateAsync(entityInstance.Entity, originalETag);
    /// // This fails as the etags no longer match (we didn't stash the result)
    /// entityInstance.Entity.SomeValue = "A less formal 'yo'.";
    /// await client.UpdateAsync(entityInstance.Entity, originalETag);
    /// </code>
    /// </para>
    /// </example>
    public interface ICosmosDbSqlClient
    {
        /// <summary>
        /// Gets the connection policy for the client.
        /// </summary>
        ...
        /// <summary>
        /// Gets the default throughput RUs for the client.
        /// </summary>
        int? DefaultOfferThroughput { get; }

        /// <summary>
        /// Gets a value indicating whether the client is using database-level throughput.
        /// </summary>
        ...
        /// <summary>
        /// Gets the desired consistency level for the client.
        /// </summary>
        ConsistencyLevel? DefaultDesiredConsistencyLevel { get; }

        /// <summary>
        /// Gets the indexing policy for the client.
        /// </summary>
        IndexingPolicy DefaultIndexingPolicy { get; }

        /// <summary>
        /// Gets the JsonSerializerSettings for the client.
        /// </summary>
        ...
        /// <summary>
        /// Gets the unique key policy for the client.
        /// </summary>
        UniqueKeyPolicy DefaultUniqueKeyPolicy { get; }

        /// <summary>
        /// Gets the document client underlying the client.
        /// </summary>
        ...
        /// <summary>
        /// Delete the collection related to this client.
        /// </summary>
        ...
        /// <summary>
        /// Replaces the existing Cosmos DB indexing policy on the client.
        /// </summary>
